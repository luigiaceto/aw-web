...
from aw_cli.web.services import default_provider_name, get_cover
from aw_cli.web.styles import CSS
from aw_cli.web.utils import anime_to_json, esc, has_new_episode, q
...
def page(title: str, body: str) -> bytes:
    ...
    <head>
      ...
      <title>{esc(title)} - aw-cli web</title>
      <style>{CSS}</style>
    </head>
    <body>
      <header class="topbar">
        <a class="brand" href="/">aw-cli web</a>
...
def watch_card(item: dict[str, Any], latest: list[Anime]) -> str:
    ...
    <article class="card saved-card">
      ...
      <div class="card-body">
        ...
        <p>Sei arrivato all'episodio <strong>{esc(item['current_episode'])}</strong> / {esc(item['last_episode'])}</p>
        <div class="row-actions">
          {quick_play_form(str(item['provider']), anime, str(item['current_episode']), 'Riprendi')}
          {browser_play_form(str(item['provider']), anime, str(item['current_episode']), 'Browser')}
...
def episode_row(provider_name: str, anime: Anime, episode: Anime.Episode, current_episode: str) -> str:
    active = " active" if episode.num == current_episode else ""
    return f"""
